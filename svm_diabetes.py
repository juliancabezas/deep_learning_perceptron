...
def main():
    ...
    if not os.path.exists('Results/grid_search_svm.csv'):
        ...
        for cost in cost_array:
            ...
            for kernel in kernel_array:
                ...
                svm = SVC(random_state=124, kernel=kernel, C = cost)
        ...
        dic = {'cost':cost_full,'kernel':kernel_full,'Accuracy':acc_full,'Kappa':kappa_full,'F1':f1_full}
        df_grid_search = pd.DataFrame(dic)
        df_grid_search.to_csv('Results/grid_search_svm.csv')
        print("Tuning Ready!")

    else:
        # In case the parameters were already tuned
        df_grid_search = pd.read_csv('Results/grid_search_svm.csv')
    ...
    cost_max = int(df_grid_search['cost'].values[row_max])
    kernel_max = str(df_grid_search['kernel'].values[row_max])
    ...
    svm = SVC(random_state=124, kernel=kernel_max, C = cost_max)
    ...
    df_results.to_csv('Results/results_svm.csv')
